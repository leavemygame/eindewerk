...
else
{
    ...
    if ($pasw == $passwordcheck)
    {
    //controleer of de gebruikersnaam al bestaat
        $query = "SELECT * FROM inloggegevens WHERE Username = '".$username."'";
        $result = mysqli_query($conn, $query) or die(mysqli_error($conn));
        $count = mysqli_num_rows($result);

        if ($count == 0)//zorg dat een gebruiker uniek is, bestaat er al een gelijkaardig record?
        {
            $query = "INSERT INTO inloggegevens (Username,Password,Type) VALUES ('".$user."','".$pasw."','2')";
            $result = mysqli_query($conn, $query) or die(mysqli_error($conn));


            $query = "SELECT * FROM inloggegevens WHERE Username='".$username."' and Password = '".$password."'";
            $result = mysqli_query($conn, $query) or die (mysqli_error($conn));
            $count = mysqli_num_rows($result);
